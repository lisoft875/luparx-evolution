...
const PARKING_ERROR_KEYS: Record<string, TranslationKey> = {
  ...
  SESSION_NOT_FOUND: 'citizen.parking.error.SESSION_NOT_FOUND',
};
...
export function vehiclePlateError(error: unknown, t: (key: TranslationKey) => string): string | undefined {
  ...
  if (error.code === 'PLATE_ALREADY_REGISTERED') return t('citizen.vehicles.form.error.PLATE_ALREADY_REGISTERED');
  return undefined;
}
